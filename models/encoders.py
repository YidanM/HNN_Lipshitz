...
class HNN(Encoder):
    ...
    def encode0(self, x, noise_std): # add noise perturbations to hyperbolic features
        x_hyp = self.manifold.proj(self.manifold.expmap0(self.manifold.proj_tan0(x, self.c), c=self.c), c=self.c).cuda()
        x_hyp_clean = x_hyp
        if self.manifold.name == 'Hyperboloid':
            noise = torch.normal(0, noise_std, size=[(x_hyp.shape)[0], (x_hyp.shape[1])-1]).cuda()
            # need to ensure the data points still lie on the manifold after adding the noise
            x_s = x_hyp[:,1:]
            x_s += noise
            x_t = x_hyp[:,0]
            x_s_squared = x_s**2
            x_s_squared_sum = torch.sum(x_s_squared, dim = 1)
            x_t = (x_s_squared_sum+1)**0.5
            x_hyp[:,0] = x_t
        if self.manifold.name == 'PoincareBall':
            noise = torch.normal(0, noise_std, size=x_hyp.shape).cuda()
            x_hyp += noise 
            # need to ensure the data points still lie on the manifold after adding the noise
            x_hyp = self.manifold.proj(x_hyp, c=self.c)

        return x_hyp_clean, x_hyp

    def encode(self, x_hyp, adj):
...
class HGCN(Encoder):
    ...
    def encode0(self, x, noise_std): # add noise perturbations before training
